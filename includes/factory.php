...
class WPMeetupFactory {
	...
	public function retrieve_and_store($slug) {
		$events = $this->core->api->get_events($slug);
		if (! empty($events)) {
	...
	public function store_individual($event) {
		...
		if (empty($id)) {
			...
			// Now lets get the ID of the newly added event and use that for the event filtering.
			$this->core->event_db->select('id');
			$this->core->event_db->where('wpm_event_id', $event->id);
			$id = $this->core->event_db->get();
			$id = $id[0];
		...
		foreach ($event_array as $event) {
			...
			if ($event_raw) {
				...
				$post_array = array(
					...
					'post_content' => $this->build_meetup_backlink($event) . (! empty($event_raw->description) ? $event_raw->description : ''),
					//'post_name' => $post_name,
